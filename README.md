...
This is not hypothetical on a libhybris device: Xwayland aborts at startup when
it cannot take the single HWC2 composer client the compositor already holds, and
the next focus change locks up the session.
...
monotonic timestamp — the same value mutter's own no-X11 fallback would use.
...
The proper fix belongs in mutter — a Wayland window's focus should never depend
on a live X server, and the wait should be bounded there. This exists because
rebuilding the compositor is not always practical on a device whose libmutter
carries downstream patches.
